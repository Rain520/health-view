            ...
            <el-row v-for="(comment, index) in commentList " :key="index" style="padding: 10px 0;">
                ...
                <el-row style="padding: 8px 0;">
                    <el-col :span="22" :offset="2">
                        ...
                        <span @click="toggleReplyInput(comment, 'showReplyInput')"
                            style="cursor: pointer;margin-left: 15px;font-size: 14px;color: #8A919F;user-select: none;">
                        ...
                        <span @click="upvote(comment)"
                            style="cursor: pointer;margin-left: 15px;font-size: 14px;color: #8A919F;user-select: none;">
                            <i class="el-icon-discount" v-if="!comment.upvoteFlag">点赞({{ comment.upvoteCount }})</i>
                    ...
                    <el-row>
                        <el-col :span="22" :offset="2">
                            ...
                            <el-row style="padding: 10px 0;">
                                ...
                                <span @click="toggleReplyInput(commentChild, 'replyInputStatus')"
                                    style="cursor: pointer;margin-left: 15px;font-size: 14px;color: #8A919F;user-select: none;">
                                    <i class="el-icon-chat-dot-round"></i>
                                    回复
                                </span>
                                <span @click="upvote(commentChild)"
                                    style="cursor: pointer;margin-left: 15px;font-size: 14px;color: #8A919F;user-select: none;">
                                    <i class="el-icon-discount" v-if="!commentChild.upvoteFlag">点赞({{ commentChild.upvoteCount }})</i>
    ...
    methods: {
        ...
        upvote(comment) {
            ...
            if (comment.upvoteFlag) {
                ...
                if (index !== -1) {
                    ...
                    comment.upvoteCount -= 1; // 减少点赞数
                }
            } else {
                // 点赞
                if (!upvoteList.includes(this.userData.id.toString())) {
                    upvoteList.push(this.userData.id.toString()); // 添加用户ID
                    comment.upvoteCount += 1; // 增加点赞数
                }
            }

            // 更新服务器上的点赞数据
            let evaluations = {
                flag: comment.upvoteFlag,
                content: comment.content,
                id: comment.id,
                upvoteList: upvoteList.length ? upvoteList.join(',') : null
            };

            this.$axios.put(`evaluations/update`, evaluations)
                .then(res => {
                    if (res.data.code == 200) {
                        // 更新upvoteList和upvoteFlag
        ...
        onFocus() {
            ...
            this.commentList.forEach(entity => {
                ...
                // 子级评论
                entity.commentChildVOS.forEach(entity => {
                    entity.replyInputStatus = false
                });
            });
        },
        // 输入框失去焦点
        ...
        toggleReplyInput(comment, field) {
            this.isFocused = false;
